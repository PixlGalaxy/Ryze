...
class voice(commands.Cog):
            ...
            try:
                if after.channel.id == voiceID:
                    ...
                    if cooldown is None:
                        ...
                        await member.send("You are creating channels too quickly you've been put on a 15 second cooldown!")
    ...
    async def help(self, ctx):
        embed = discord.Embed(title="Ryze Help", description="", color=0xFF7B00)
        embed.set_author(
        ...
        commands_description = [
            ("--------------------------------------------------------------", ""),
            ("Setup managed voice channel for server using the following command:", "`>voice setup`"),
            ("--------------------------------------------------------------", ""),
            ("Lock your channel by using the following command:", "`>voice lock`"),
            ("--------------------------------------------------------------", ""),
            ("Unlock your channel by using the following command:", "`>voice unlock`"),
            ("--------------------------------------------------------------", ""),
            ("Change your channel name by using the following command:", "`>voice name <name>`\n**Example:** `>voice name My Channel`"),
            ("--------------------------------------------------------------", ""),
            ("Change your channel limit by using the following command:", "`>voice limit number`\n**Example:** `>voice limit 2`"),
            ("--------------------------------------------------------------", ""),
            ("Give users permission to join by using the following command:", "`>voice permit @person`\n**Example:** `>voice permit @Galaxy#4625`"),
            ("--------------------------------------------------------------", ""),
            ("Claim ownership of channel once the owner has left:", "`>voice claim`\n**Example:** `>voice claim`"),
            ("--------------------------------------------------------------", ""),
            ("Remove permission and the user from your channel using the following command:", "`>voice reject @person`\n**Example:** `>voice reject @PIXL_WOLF`"),
            ("--------------------------------------------------------------", "") 
        ]

        for name, value in commands_description:
            embed.add_field(name=name, value=value, inline=False)
        
        invite_link = "[Click here](https://discord.com/oauth2/authorize?client_id=998006513559941170&permissions=8&scope=bot) to invite Ryze to your server."
        embed.add_field(name="Invite Link:", value=invite_link, inline=False)

        embed.set_footer(text='Hosted On Zaylar Server, www.itzgalaxy.com')
    ...
    async def setup(self, ctx):
        ...
        if ctx.author.id == ctx.guild.owner_id or ctx.author.id == 151028268856770560:
            ...
            await ctx.channel.send("**You have 60 seconds to finish the setup!**")
            ...
            await ctx.channel.send(f"{ctx.author.mention} only the owner of the server can setup Ryze!")
        conn.commit()
        conn.close()
